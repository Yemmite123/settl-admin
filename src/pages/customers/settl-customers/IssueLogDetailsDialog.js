...
export default function IssueLogDetailsDialog(props) {
        ...
        <div className="mt-5">
          {information.map((info) => (
            <div className="d-flex mb-2" key={info.id}>
              <div className="w-50 font-weight-bold">{info.title}</div>
              <div className="w-50">{info.value}</div>
            </div>
          ))}
        </div>
        <hr />
